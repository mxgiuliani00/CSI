function [fval,dummy] = objFunWQEISS_regression_2obj(X,varargin)
...
if ~isempty(archiveIx);
    ...
    if cardinality > maxCardinality
        ...
        fval = [Inf,numel(featIxes),Inf,Inf];
    elseif cardinality == 0
        ...
        [SU,traj] = trainAndValidateELM_regression(PHI,Y,featIxes,nFolds,nELM,nUnits);
        
        % fitness values (- for those obj. functions to maximize)                
        fval = [-SU,cardinality,-REL,RED];            
        % add solution to archive and fvals
        archive = cat(1,archive,featIxes);
        fvals   = cat(1,fvals,[-SU,cardinality,-REL,RED]);
